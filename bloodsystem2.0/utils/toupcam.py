...
TOUPCAM_FLAG_LIGHTSOURCE         = 0x0000080000000000  # light source
...
TOUPCAM_OPTION_FRONTEND_DEQUE_LENGTH  = 0x31       # frontend frame buffer deque length, range: [2, 1024], default: 4
...
TOUPCAM_OPTION_OPEN_USB_ERRORCODE     = 0x3b       # get the open usb error code
...
TOUCPAM_OPTION_EVENT_HARDWARE         = 0x04000000 # enable or disable hardware event: iOption = TOUCPAM_OPTION_EVENT_HARDWARE | (event type), iValue = 1 (enable), 0 (disable); default: disable
...
class Toupcam:
    ...
    @classmethod
    def Version(cls):
        """get the version of this dll, which is: 50.19879.20211123"""
        cls.__initlib()
    ...
    @classmethod
    def HotPlug(cls, fun):
        """only available on macOS and Linux, it's unnecessary on Windows"""
        if sys.platform == 'win32':
    ...
    def Flush():
        """Flush is obsolete, it's a synonyms for put_Option(h, TOUPCAM_OPTION_FLUSH, 3)"""
